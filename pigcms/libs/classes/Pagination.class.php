...
class Pagination
{
    ...
    protected function _get_link($page,$text)
    {
        switch ($this->method) {
            case 'ajax':
                ...
                return '<li class="ng-scope"><a onclick="' . $this->ajax_func_name . '(\'' . $page . '\''.$parameter.')" href="javascript:void(0)">' . $text . '</a></li>' . "\n";
            break;
             
            case 'html':
                $url = str_replace('?', $page,$this->parameter);
                return '<li class="ng-scope"><a href="' .$url . '">' . $text . '</a></li>' . "\n";
            break;
             
            default:
                return '<li class="ng-scope"><a href="' . $this->_get_url($page) . '">' . $text . '</a></li>' . "\n";
            break;
        }
    }
    ...
    protected function show_2()
    {
        if($this->total_pages != 1)
        {
            ...
            for($i = 1;$i<=$this->total_pages;$i++)
            {
                if($i == $this->now_page)
                {
                    $return .= "<li class='ng-scope'><a class='now_page'>$i</a></li>\n";
                }
                else
                {
    ...
    protected function show_1()
    {
        ...
        for ($i = $begin; $i <= $begin + $plus * 2;$i++)
        {
            ...
            if($i == $this->now_page)
            {
                $return .= "<li class='ng-scope active'><a class='now_page'>$i</a></li>\n";
            }
            else
            {
                $return .= $this->_get_link($i, $i) . "\n";
