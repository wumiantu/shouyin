...
function pages($total,$page,$pageSize,$url){
	...
	if ($total>$pageSize){
		...
		$str.=$preStr.$nextStr.'</td><td align="right" style="text-align:right;" valign="top">';
		$pageOption='';
 ...
 */
function s_showMessage($tip,$url='',$interval=2000){
	$seconds=$interval/1000;
	if ($url){
		$r='<div style="color:#888">'.$seconds.'秒后将自动跳转...如果您的浏览器不能跳转<br/><a href="'.$url.'">请点击这里</a></div>';
...
function showMessage($tip,$url='',$interval=2000){
	...
	if ($url){
		...
		$iconUrl='/pigcms_tpl/Home/Static/images/loading.gif';
	}else {
		$r='';
		$interval=100000000000000000;
		$iconUrl='/pigcms_tpl/Home/Static/images/success.png';
	}
	echo '<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
<html xmlns="http://www.w3.org/1999/xhtml">
<head>
<meta http-equiv="Content-Type" content="text/html; charset='.CHARSET.'" />
<link href="/pigcms_tpl/Home/Static/css/tipsstyle.css" type="text/css" rel="stylesheet">
<script src="/pigcms_tpl/Home/Static/js/mootools1.3.js"></script>
<script src="/pigcms_tpl/Home/Static/js/mootools-more.js"></script>
<title></title>
<script>
window.addEvent(\'domready\',function(){
	(function(){window.location.href=\''.$url.'\';}).delay('.$interval.');
